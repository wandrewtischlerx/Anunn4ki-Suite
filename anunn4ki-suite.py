...
print(f"                   _T/L   {Fore.GREEN}Suite 0.1{Style.RESET_ALL}{Fore.YELLOW}  _LT|L/_|__L_|_L_")
...
def gerar_ip_aleatorio():
    """Gera um endereço IP aleatório dentro do intervalo possível."""
...
def verificar_servico(ip, porta, servico):
    """Verifica se um serviço específico está rodando na porta fornecida, independentemente da versão."""
    ...
    if banner:
        ...
        if any(keyword in banner_upper for keyword in servico_upper.split()):
            return True, banner
    return False, banner
...
def tarefa_por_servico(ip, porta, servico, resultados):
    """Executa a tarefa de testar serviços em um IP e coleta o banner se o serviço estiver presente."""
    if testar_porta(ip, porta):
        with print_lock:
            print(f"{Fore.GREEN}Porta {porta} está aberta em {ip}.{Style.RESET_ALL}")
        encontrado, banner = verificar_servico(ip, porta, servico)
        if encontrado:
            with print_lock:
                print(f"{Fore.GREEN}O serviço {servico} está rodando na porta {porta} em {ip}.{Style.RESET_ALL}")
                print(f"{Fore.YELLOW}Banner: {banner}{Style.RESET_ALL}")
            resultados.append((ip, banner))
            return True
        else:
            with print_lock:
                print(f"{Fore.RED}O serviço {servico} NÃO está rodando na porta {porta} em {ip}.{Style.RESET_ALL}")
                if banner:
                    print(f"{Fore.YELLOW}Banner: {banner}{Style.RESET_ALL}")
    else:
        with print_lock:
...
def main():
    ...
    if opcao == "1":
        porta = int(input("Digite o número da porta a ser testada: "))
        numero_de_ips = int(input("Digite o número de IPs a serem testados: "))
        ...
        threads = [threading.Thread(target=buscar_por_porta) for _ in range(10)]  # Ajuste o número de threads conforme necessário
        for t in threads:
            t.start()
        for t in threads:
            t.join()
        ...
        def buscar_por_servico():
            ...
            while encontrado < numero_de_ips:
                ip = gerar_ip_aleatorio()
                if tarefa_por_servico(ip, porta, servico, resultados):
                    encontrado += 1

        # Cria e inicia múltiplas threads para buscar o serviço
        threads = [threading.Thread(target=buscar_por_servico) for _ in range(10)]  # Ajuste o número de threads conforme necessário
        for t in threads:
            t.start()
        for t in threads:
            t.join()
        ...
        if resultados:
            ...
            print(f"{Fore.RED}Nenhum IP encontrou o serviço {servico}.{Style.RESET_ALL}")
